...
class RosDriver:
    def __init__(self):
        ...
        # 로봇 동작 관련 속성 (속도, 회전)
        self.SPEED_VALUE = 0.45  # 기본 속도
        self.TURN_VALUE = 0.25  # 회전값
        self.speed = 0.4 # 현재 속도(수동에서 씀)
        # 빨간색 HSV 범위 (두 개로 설정)
        self.Hmin1, self.Smin1, self.Vmin1 = 0, 100, 100    # 첫 번째 빨강 범위
        self.Hmax1, self.Smax1, self.Vmax1 = 10, 255, 255

        self.Hmin2, self.Smin2, self.Vmin2 = 170, 100, 100  # 두 번째 빨강 범위
        self.Hmax2, self.Smax2, self.Vmax2 = 180, 255, 255
    ...
    def colorDisplay(self, image):
        ...
        # 빨간색 두 개의 범위 마스크 생성
        lower_red1 = np.array([self.Hmin1, self.Smin1, self.Vmin1], dtype=np.uint8)
        upper_red1 = np.array([self.Hmax1, self.Smax1, self.Vmax1], dtype=np.uint8)
        mask1 = cv.inRange(hsv, lower_red1, upper_red1)

        lower_red2 = np.array([self.Hmin2, self.Smin2, self.Vmin2], dtype=np.uint8)
        upper_red2 = np.array([self.Hmax2, self.Smax2, self.Vmax2], dtype=np.uint8)
