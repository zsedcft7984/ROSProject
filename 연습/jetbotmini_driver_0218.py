...
class TransbotDriver:
    ...
    def colorDisplay(self, image):
        ...
        if len(cnts) > 0:
            ...
            if color_radius > 20:  # Only consider sufficiently large contours
                cv.circle(image, (int(color_x), int(color_y)), int(color_radius), (255, 0, 255), 2)
                color_x = (320 - color_x) / 320  # Calculate error based on center of the frame
                
                rospy.loginfo(f"Color detected at x: {color_x}, y: {color_y}, radius: {color_radius}")
                    
                if mot_start == 1:
                    self.speed_value = 0.35
                    self.turn_value = 0.05
                    ...
                    rospy.loginfo(f"Motor speeds - Left: {left_speed}, Right: {right_speed}")

                    # Send motor control signals via I2C
                    try:
                        bus.write_i2c_block_data(ADDRESS, 0x01, [1, left_speed, 1, right_speed])  # Forward motion
                        rospy.loginfo(f"Sent motor command - Left: {left_speed}, Right: {right_speed}")
